...
export default function NurseAppointmentsPage() {
  ...
  return (
    <div className="px-4 pb-4">
      <AppHeader greeting="Appointments" subtitle="Patient schedules" />
      {loading ? (
        ...
        <div className="space-y-2">
          {appointments.map((a) => (
            <div key={a._id} className="card p-4">
              ...
              <p className="text-xs text-gray-500">{a.doctorName} · {a.specialization}</p>
              <p className="text-xs text-primary mt-2 font-medium">{new Date(a.date).toLocaleDateString()} at {a.time}</p>
            </div>
          ))}
        </div>
      )}
